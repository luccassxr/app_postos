...
class AppController extends ChangeNotifier {
  AppController(this.repository, {bool useFirebaseAuth = false})
  ...
  Future<void> initialize() async {
    ...
    for (final item in repository.users) {
      if (item.id == session) {
        _user = item;
        break;
      }
  ...
  Future<void> register({
    ...
    if (firebaseAuth != null) {
      _user = await firebaseAuth!.register(
        name: name,
        cpf: cpf,
        phone: phone,
        email: email,
  ...
  Future<TransactionModel> recordDemoFueling({
    ...
    final item = TransactionModel(
      ...
      fuel: fuel,
      liters: liters,
      points: loyalty.pointsForFueling(amount),
      type: TransactionType.fueling,
    );
  ...
  Future<TransactionModel> redeem(CouponModel coupon) async {
    if (_user == null) throw StateError('Nenhum cliente autenticado.');
    couponService.validateRedemption(
      coupon,
      balance: points,
    ...
    await repository.saveTransactions([...repository.transactions, item]);
    await repository.saveRedeemedCoupons({
      ...repository.redeemedCouponKeys,
      '${_user!.id}:${coupon.id}',
    });
    notifyListeners();
    return item;
